...
def main():
    parser = argparse.ArgumentParser(description='Run inference with Llama-3-8B-UltraMedical on medical datasets')
    ...
    for dataset_path in args.dataset_paths:
        ...
        with open(f"results/{dataset_name}/predictions.csv", 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['input', 'reference', 'prediction'])
            for i in range(len(predictions)):
                writer.writerow([inputs_df.iloc[i]['inputs'], references[i], predictions[i]])
        
        with open(f"results/{dataset_name}/metrics.json", 'w') as f:
    ...
    with open("results/overall_metrics.json", 'w') as f:
